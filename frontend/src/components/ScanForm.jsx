...
export default function ScanForm({
  ...
  const inputClass = 'w-full rounded-xl sm:rounded-2xl pl-10 sm:pl-12 pr-4 sm:pr-5 py-3.5 sm:py-4 text-sm sm:text-base border focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all shadow-lg bg-gray-950/80 border-white/10 text-white placeholder-gray-400';

  /* Glassmorphism: Dark mode only */
  const glassCard = 'rounded-2xl sm:rounded-3xl border backdrop-blur-xl shadow-2xl bg-gray-900/60 border-white/10 shadow-black/20';
  const labelColor = 'text-cyan-400';
  const hintColor = 'text-gray-400';
    ...
    >
      <div className="relative max-w-4xl mx-auto text-center">
        <motion.h2
          ...
          className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-slate-200 mb-2"
        >
        ...
        >
          <form onSubmit={onSubmit} className="relative space-y-6 text-left">
            ...
            <div className="flex flex-col items-center gap-2 sm:gap-3">
              {isTurnstileDevMode ? (
                <div className="flex items-center justify-center p-2 sm:p-4 rounded-xl border-2 border-amber-500/40 bg-amber-900/20 w-full max-w-[320px] min-h-[65px]">
                  <span className="text-amber-400 text-sm">Dev mode — verification skipped</span>
                </div>
              ) : (
                <>
                  <div
