...
const ScanTerminal = ({ logs = [] }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto rounded-xl bg-black/90 overflow-hidden font-mono">
      <div className="flex items-center gap-2 px-3 py-2 bg-slate-900/80">
        <span className="w-2.5 h-2.5 rounded-full bg-red-500" />
        <span className="w-2.5 h-2.5 rounded-full bg-amber-500" />
        <span className="w-2.5 h-2.5 rounded-full bg-green-500" />
        <span className="text-green-500/80 text-xs ml-2">scan_session</span>
      </div>
      <div className="p-4 min-h-[180px] max-h-[300px] overflow-y-auto custom-scrollbar text-green-400 text-sm sm:text-base" style={{ color: '#00ff00' }}>
        {logs.length === 0 && (
